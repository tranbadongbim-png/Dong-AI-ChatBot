...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div
      ...
      className="mx-auto flex w-full max-w-4xl flex-col items-center justify-center px-4 py-6 sm:py-8 text-center"
    >
      {/* Icon & Title */}
      <div className="mb-3 sm:mb-4 inline-flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20 ring-4 ring-blue-50 shrink-0">
      ...
      <div className="mt-6 sm:mt-8 grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        >
          <div className="flex items-center justify-between w-full mb-2">
            ...
            <span
              className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold whitespace-nowrap shrink-0 ${
                enableThinking
      ...
      <div className="mt-6 sm:mt-8 w-full text-left">
        <div className="mb-3 flex items-center justify-between px-1">
          ...
          <span className="text-xs text-slate-400 whitespace-nowrap">Nhấp vào để thử ngay</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex w-full items-center justify-between">
                  <span
                    className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-semibold whitespace-nowrap shrink-0 ${
                      isThinking
                        ? "bg-amber-100 text-amber-900 border border-amber-200"
                        : "bg-blue-50 text-blue-700 border border-blue-200"
                    }`}
                  >
                    {isThinking ? <Brain className="h-3 w-3 shrink-0 text-amber-600" /> : <Zap className="h-3 w-3 shrink-0 text-blue-600" />}
                    <span className="whitespace-nowrap">{preset.category}</span>
                  </span>
                  <ArrowRight className="h-3.5 w-3.5 text-slate-400 opacity-0 transition-all group-hover:translate-x-0.5 group-hover:opacity-100 shrink-0" />
                </div>

                <h4 className="mt-2 text-xs font-bold text-slate-800 group-hover:text-blue-600 line-clamp-1">
                  {preset.title}
                </h4>
